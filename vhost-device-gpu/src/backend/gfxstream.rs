...
impl Renderer for GfxstreamAdapter {
    ...
    fn attach_backing(
        ...
        let rutabaga_iovecs =
            Self::sglist_to_rutabaga_iovecs(&vecs[..], mem).map_err(|_| GpuResponse::ErrUnspec)?;
...
mod gfx_fence_tests {
    ...
    fn new_gpu() -> GfxstreamAdapter {
        ...
        let fence = GfxstreamAdapter::create_fence_handler(vring.clone(), fence_state.clone());

        let builder = GfxstreamAdapter::configure_rutabaga_builder(&config, fence);
        let rutabaga = builder.0.build().unwrap();

        // 5) **install into TLS** so Renderer methods can find it
        TLS_RUTABAGA.with(|slot| {
    ...
    rusty_fork_test! {
        ...
        fn test_gpu_capset() {
            ...
            // let capset_info = gfxstream_gpu.get_capset_info(0);
            test_capset_operations(&mut gfxstream_gpu, 0);
            ...
            // let capset_info = gfxstream_gpu.get_capset(capset_id, version);
    ...
    fn fence_handler_global_and_context_paths() {
        ...
        let fence_state_a = Arc::new(Mutex::new(FenceState {
            ...
            completed_fences: Default::default(),
        }));

        let handler_a =
            GfxstreamAdapter::create_fence_handler(vring_a.clone(), fence_state_a.clone());
        ...
        // -------------------------------
        // B) Context ring: no match → completed_fences updated, no notify
        // -------------------------------
        ...
        let fence_state_b = Arc::new(Mutex::new(FenceState {
            descs: vec![fence_desc(VirtioGpuRing::Global, 7, 1, 1)], /* only global → no context
                                                                      * match */
            completed_fences: Default::default(),
        }));

        let handler_b =
            GfxstreamAdapter::create_fence_handler(vring_b.clone(), fence_state_b.clone());
    ...
    fn calculate_size_overflow_width_height() {
        ...
        assert!(r.calculate_size().is_err());
    }
    ...
    fn sglist_to_rutabaga_iovecs_err_on_any_bad_segment() {
        ...
        let sg = vec![(GuestAddress(0x4000), 16usize)];

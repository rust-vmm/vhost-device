...
impl VhostUserConsoleBackend {
    ...
    pub fn assign_input_method(&mut self, vm_sock_addr: String) -> Result<()> {
        ...
        match backend_type {
            ...
            BackendType::Network => {
                let tcp_listener = TcpListener::bind(&vm_sock_addr).map_err(|e| {
                    eprintln!("Failed to bind TCP listener to {}: {}", vm_sock_addr, e);
                    Error::CreateBackendFailed
                })?;
            ...
            BackendType::Uds => {
                ...
                if let Err(e) = std::fs::remove_file(&vm_sock_addr) {
                    if e.kind() != std::io::ErrorKind::NotFound {
                        eprintln!(
                            "Failed to remove existing UDS socket at {}: {}",
                            vm_sock_addr, e
                        );
                        return Err(Error::CreateBackendFailed);
                    }
                }
                let uds_listener = UnixListener::bind(&vm_sock_addr).map_err(|e| {
                    eprintln!("Failed to bind UDS listener to {}: {}", vm_sock_addr, e);
                    Error::CreateBackendFailed
                })?;
    ...
    pub fn prepare_exit(&self) {
        ...
        match backend_type {
            ...
            BackendType::Uds => {
                ...
                if let Some(listener) = &self.uds_listener {
                    if let Ok(addr) = listener.local_addr() {
                        if let Some(path) = addr.as_pathname() {
                            if let Err(e) = std::fs::remove_file(path) {
                                println!(
                                    "Failed to remove UDS socket file {}: {}",
                                    path.display(),
                                    e
                                );
                            } else {
                                println!("Removed UDS socket file: {}", path.display());
                            }
                        }
                    }
                }

...
impl MediaAllocator {
        ...
        T: IntoIterator<Item = AddressRange>,
    {
        ...
        for r in pools.into_iter().filter(|p| !p.is_empty()) {
            regions.insert(r);
        }
        Ok(MediaAllocator {
            min_align,
...
mod tests {
    ...
    fn test_allocator_new_from_list() {
        ...
        assert!(alloc.regions.contains(&pools[0]));
        assert!(alloc.regions.contains(&pools[1]));
    }
    ...
    #[test]
    fn test_allocator_allocate_misaligned_pool_start() {
        // Pool starts at address 2 with min_align=4. The allocator must bump
        // the first allocation to the next 4-aligned address (4), exercising
        // the `r => slot.start + min_align - r` branch in `allocate`.
        let pool = AddressRange::from_range(2, 1023);
        let mut allocator = MediaAllocator::new(pool, Some(4)).unwrap();
        let offset = allocator.allocate(8, 1).unwrap();
        assert_eq!(offset, 4);
        assert_eq!(
            allocator.allocs.get(&1),
            Some(&AddressRange::from_range(4, 11))
        );
        // The unaligned prefix [2, 3] should be left as a free region.
        assert!(allocator.regions.contains(&AddressRange::from_range(2, 3)));
    }

...
#[derive(Copy, Clone, FromBytes, AsBytes, PartialEq, Eq)]
#[repr(C)]
        ...
        let len = match *self {
            Self::OkDisplayInfo(ref info) => {
                ...
                for (disp_mode, &(width, height, enabled)) in disp_info.pmodes.iter_mut().zip(info)
                {
                    ...
                    disp_mode.enabled = u32::from(enabled);
                }
                writer
                    .write_obj(disp_info)
...
mod tests {
    ...
    #[test]
    fn test_get_display_info_debug() {
        let get_display_info = GpuCommand::GetDisplayInfo;
        assert_eq!(format!("{:?}", get_display_info), "GetDisplayInfo");
    }

    #[test]
    fn test_get_edid_debug() {
        let get_edid = GpuCommand::GetEdid(virtio_gpu_get_edid::default());
        assert_eq!(format!("{:?}", get_edid), "GetEdid");
    }

    #[test]
    fn test_resource_create_2d_debug() {
        let resource_create_2d =
            GpuCommand::ResourceCreate2d(virtio_gpu_resource_create_2d::default());
        assert_eq!(format!("{:?}", resource_create_2d), "ResourceCreate2d");
    }

    #[test]
    fn test_resource_unref_debug() {
        let resource_unref = GpuCommand::ResourceUnref(virtio_gpu_resource_unref::default());
        assert_eq!(format!("{:?}", resource_unref), "ResourceUnref");
    }

    #[test]
    fn test_set_scanout_debug() {
        let set_scanout = GpuCommand::SetScanout(virtio_gpu_set_scanout::default());
        assert_eq!(format!("{:?}", set_scanout), "SetScanout");
    }

    #[test]
    fn test_set_scanout_blob_debug() {
        let set_scanout_blob = GpuCommand::SetScanoutBlob(virtio_gpu_set_scanout_blob::default());
        assert_eq!(format!("{:?}", set_scanout_blob), "SetScanoutBlob");
    }

    #[test]
    fn test_resource_flush_debug() {
        let resource_flush = GpuCommand::ResourceFlush(virtio_gpu_resource_flush::default());
        assert_eq!(format!("{:?}", resource_flush), "ResourceFlush");
    }

    #[test]
    fn test_transfer_to_host_2d_debug() {
        let transfer_to_host_2d =
            GpuCommand::TransferToHost2d(virtio_gpu_transfer_to_host_2d::default());
        assert_eq!(format!("{:?}", transfer_to_host_2d), "TransferToHost2d");
    }

    #[test]
    fn test_resource_detach_backing_debug() {

...
impl VirglRendererAdapter {
    pub fn new(queue_ctl: &VringRwLock, config: GpuConfig, gpu_backend: GpuBackend) -> Self {
        let virglrenderer_flags = VirglRendererFlags::new()
            .use_virgl(true)
            .use_venus(true) // todo: config.capsets().contains(GpuCapset::VENUS)
            //.use_drm(true) //todo: config.capsets().contains(GpuCapset::DRM)
...
impl Renderer for VirglRendererAdapter {
    ...
    fn attach_backing(
        ...
        // (Optional) ensure we won’t reallocate later
        // iovs.shrink_to_fit(); // do this BEFORE the FFI call if you use it
    ...
    fn update_cursor(
        ...
        let data = vec![0u8; config.expected_buffer_len()].into_boxed_slice();
    ...
    fn create_fence(&mut self, fence: RutabagaFence) -> VirtioGpuResult {
        self.renderer
            .create_fence(fence.fence_id as u32, fence.ctx_id)
            .map_err(|_| ErrUnspec)?;
        Ok(OkNoData)
    }
    ...
    fn set_scanout(
        ...
        let info_3d = resource.virgl_resource.info_3d.as_ref().ok_or_else(|| {
            error!("resource {} has handle but no info_3d", resource_id);
            ErrUnspec
        })?;
        ...
        if frontend_supports_dmabuf2 {
            ...
            self.gpu_backend
                ...
                .map_err(|e| {
                    error!(
                        "Failed to send VHOST_USER_GPU_DMABUF_SCANOUT2 for resource {}: {e:?}",
                        resource_id
                    );
                    ErrUnspec
                })?;
            ...
            self.gpu_backend
                ...
                .map_err(|e| {
                    error!(
                        "Failed to send VHOST_USER_GPU_DMABUF_SCANOUT for resource {}: {e:?}",
                        resource_id
                    );
                    ErrUnspec
                })?;
...
mod virgl_cov_tests {
    ...
    rusty_fork::rusty_fork_test! {
        ...
        fn virgl_end_to_end_once() {
            ...
            // -------------------------------
            // B) Initialize virgl ONCE in this forked process; exercise adapter paths
            // -------------------------------
            ...
            assert!(gpu.update_cursor(9_999, cursor_pos, 0, 0).is_err());

            // submit_command: expected to Err with dummy buffer
            let mut cmd = [0u8; 8];
            let fence_id: Vec<u64> = vec![];
            assert!(gpu.submit_command(1, &mut cmd[..], &fence_id).is_err());
            ...
            assert!(gpu.flush_resource(9_999, dirty).is_err());
            ...
            // Test capset queries
            for index in [0, 1, 3] {
                test_capset_operations(&mut gpu, index);
            }
        }
    }
}

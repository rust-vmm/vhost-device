...
mod tests {
    ...
    fn test_vsock_config_setup_from_vm_args_unix() {
        ...
        let params = format!(
            "--vm socket={vhost3_socket},uds_path={vm3_vsock} --vm \
             socket={vhost4_socket},uds-path={vm4_vsock},guest-cid=4,tx_buffer_size=65536,\
             queue_size=1024,groups=group1 --vm \
             groups=group2+group3,guest-cid=5,socket={vhost5_socket},uds_path={vm5_vsock},\
             tx-buffer-size=32768,queue_size=256",
    ...
    fn test_vsock_config_setup_from_vm_args_vsock() {
        ...
        let params = format!(
            "--vm socket={vhost3_socket},uds_path={vm3_vsock} --vm \
             ...
             queue_size=1024,groups=group1 --vm \
             groups=group2+group3,guest-cid=5,socket={vhost5_socket},uds_path={vm5_vsock},\
             tx-buffer-size=32768,queue_size=256 --vm \
             guest-cid=6,socket={vhost6_socket},forward-cid=1,forward-listen=1234+4321,\
             queue-size=2048",
            vhost3_socket = socket_paths[0].display(),
            vhost4_socket = socket_paths[1].display(),
            vhost5_socket = socket_paths[2].display(),
    ...
    fn test_main_structs_unix() {
        ...
        assert_eq!(
            format!("{args:?}"),
            "VsockArgs { param: Some(VsockParam { guest_cid: 0, socket: \"\", uds_path: \
             Some(\"\"), tx_buffer_size: 0, queue_size: 0, groups: \"\" }), vm: None, config: \
             None }"
        );
        ...
        assert_eq!(
            format!("{param:?}"),
            "VsockParam { guest_cid: 0, socket: \"\", uds_path: Some(\"\"), tx_buffer_size: 0, \
             queue_size: 0, groups: \"\" }"
        );
        ...
        assert_eq!(
            format!("{config:?}"),
            "ConfigFileVsockParam { guest_cid: None, socket: \"\", uds_path: Some(\"\"), \
             tx_buffer_size: None, queue_size: None, groups: None }"
        );
    }
    ...
    fn test_main_structs_vsock() {
        ...
        assert_eq!(
            format!("{args:?}"),
            "VsockArgs { param: Some(VsockParam { guest_cid: 0, socket: \"\", uds_path: \
             Some(\"\"), forward_cid: None, forward_listen: None, tx_buffer_size: 0, queue_size: \
             0, groups: \"\" }), vm: None, config: None }"
        );

        let param = args.param.unwrap().clone();
        assert_eq!(
            format!("{param:?}"),
            "VsockParam { guest_cid: 0, socket: \"\", uds_path: Some(\"\"), forward_cid: None, \
             forward_listen: None, tx_buffer_size: 0, queue_size: 0, groups: \"\" }"
        );

        let args = VsockArgs::from_args_vsock(0, &PathBuf::new(), 1, "", 0, 0, "");
        assert_eq!(
            format!("{args:?}"),
            "VsockArgs { param: Some(VsockParam { guest_cid: 0, socket: \"\", uds_path: None, \
             forward_cid: Some(1), forward_listen: Some(\"\"), tx_buffer_size: 0, queue_size: 0, \
             groups: \"\" }), vm: None, config: None }"
        );

        let param = args.param.unwrap().clone();
        assert_eq!(
            format!("{param:?}"),
            "VsockParam { guest_cid: 0, socket: \"\", uds_path: None, forward_cid: Some(1), \
             forward_listen: Some(\"\"), tx_buffer_size: 0, queue_size: 0, groups: \"\" }"
        );
        ...
        assert_eq!(
            format!("{config:?}"),
            "ConfigFileVsockParam { guest_cid: None, socket: \"\", uds_path: Some(\"\"), \
             forward_cid: None, forward_listen: None, tx_buffer_size: None, queue_size: None, \
             groups: None }"
        );
        ...
        assert_eq!(
            format!("{config:?}"),
            "ConfigFileVsockParam { guest_cid: None, socket: \"\", uds_path: None, forward_cid: \
             Some(1), forward_listen: Some(\"\"), tx_buffer_size: None, queue_size: None, groups: \
             None }"
        );
    }
}

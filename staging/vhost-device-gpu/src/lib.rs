...
mod gnu_only {
    ...
    use crate::device::{self, VhostUserGpuBackend};
    ...
    impl GpuConfig {
        ...
        pub const DEFAULT_GFXSTREAM_CAPSET_MASK: GpuCapset =
            GpuCapset::GFXSTREAM_VULKAN.union(GpuCapset::GFXSTREAM_GLES);
    ...
    pub fn start_backend(socket_path: &Path, config: GpuConfig) -> Result<(), StartError> {
        ...
        let backend =
            VhostUserGpuBackend::new(config).map_err(StartError::CouldNotCreateBackend)?;
...
// Re-Export the public library function as if they weren't in a separate mod
#[cfg(target_env = "gnu")]
pub use gnu_only::*;

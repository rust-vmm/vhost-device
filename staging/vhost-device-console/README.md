...
By using the "network" backend, vhost-device-console, as soon as the guest
is booted, creates a local server in the given localhost port, and gives
the chance to connect (via a TCP connection) and interact with it.
...
```test
host# stty raw && nc 127.0.0.1 12345
```

>Note: `stty raw -echo` is used to force the tty layer to disable buffering and send / receive each character individually. After closing the connection please run `stty echo` so character are printed back on the local terminal console.

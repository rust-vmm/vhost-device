...
struct AlarmNotificationBuffer {
...
pub struct VirtioRtcDevice {
    have_alarm: bool,
    clocks: Vec<Clock>,
    alarm_notif_buffers: Vec<Arc<AlarmNotificationBuffer>>,
}
        ...
        for desc_chain in requests {
            ...
            match msg_type {
                ...
                Ok(RequestType::Read) => {
                    ...
                    let device = lock.lock().unwrap();
                    if let Some(clock) = device.clocks.get(clock_id) {
                        if let Ok(clock_reading) = clock.read() {
                            writer
                    ...
                    if let Some(alarm) = device.clocks.get(clock_id).and_then(|c| c.alarm.as_ref())
                    {
                        writer
                            .write_obj(crate::virtio_rtc::VirtioRtcRespReadAlarm {
                                alarm_time: match alarm {
                                    Alarm::Unset { .. } => 0,
                                    Alarm::Set { alarm_time, .. }
                                    | Alarm::Expired { alarm_time, .. } => *alarm_time,
                                }
                                .into(),
                ...
                Ok(RequestType::SetAlarm) if have_alarm => {
                    ...
                    if let Some((now, alarm)) = device.clocks.get_mut(clock_id).and_then(|c| {
                        let now = c.read().unwrap();
                        Some((now, c.alarm.as_mut()?))
                    }) {
                        let alarm_time = request.alarm_time.into();
                        let enabled =
                            request.flags & crate::virtio_rtc::VIRTIO_RTC_FLAG_ALARM_ENABLED > 0;
                        alarm.set(alarm_time, enabled);
                        let dur = std::time::Duration::from_nanos(alarm_time - now);
                        log::trace!("Clock {clock_id} setting alarm {dur:?} from now");
                        let mut notif = crate::virtio_rtc::VirtioRtcNotifAlarm::default();
                        notif.head.msg_type = crate::virtio_rtc::VIRTIO_RTC_NOTIF_ALARM.into();
                        ...
                        std::thread::spawn(move || {
                            ...
                            let buf = loop {
                                ...
                                device = cvar.wait(device).unwrap();
                            };

                            let mem = buf.desc_chain.memory();
                            let mut writer = buf.desc_chain.clone().writer(mem).unwrap();
                            writer.write_obj(notif).unwrap();
                            ...
                            buf.vring
                                ...
                                .unwrap();
                            buf.vring.signal_used_queue().unwrap();
                        });
                    } else {
